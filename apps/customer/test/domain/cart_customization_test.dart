...
void main() {
  test(
    'same drink with different add-ons remains a distinct configuration',
    () {
  ...
  test(
    'same drink with different temperature remains a distinct configuration',
    () {
  ...
  test('local estimate includes base, variant, option, and add-on deltas', () {
    const iced = CartLineItem(
      item: TestCatalogueRepository.latte,
    ...
    const addOnTotal = Money.fromSen(300);
    expect(iced.unitPrice(addOnTotal).sen, 1840);
    expect(iced.lineTotal(addOnTotal).sen, 3680);
  });
