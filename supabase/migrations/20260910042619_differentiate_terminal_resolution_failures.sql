-- TASK-OPS-002 / Phase 1 terminal failure semantics.
-- Live migration: 20260910042619 differentiate_terminal_resolution_failures.
-- Preserve the physical terminal credential when only the current employee loses branch access.
...
begin
  ...
  if p_credential is null or char_length(p_credential) < 40 then
    raise exception 'terminal credential is invalid'
      using errcode = '42501', detail = 'TERMINAL_CREDENTIAL_INVALID';
  end if;

  select
    t.id, t.code, sp.id, sp.code, sp.name,
    b.id, b.code, b.name, b.timezone, c.expires_at
  into
    v_terminal_id, v_terminal_code, v_sales_point_id, v_sales_point_code,
    v_sales_point_name, v_branch_id, v_branch_code, v_branch_name,
    v_timezone, v_credential_expires_at
  ...
  if not (select private.can_operate_branch(v_branch_id)) then
    raise exception 'employee is not authorized for the terminal branch'
      using errcode = '42501', detail = 'TERMINAL_BRANCH_FORBIDDEN';
  end if;
