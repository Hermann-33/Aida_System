-- TASK-PRIVACY-001 / Phase 3 — whole-account deletion must not depend on
-- a healthy membership row. A customer profile can be deleted even if member
-- provisioning is missing or degraded; any retained customer orders are
-- anonymized by trusted Auth identity before the Auth row is deleted.
