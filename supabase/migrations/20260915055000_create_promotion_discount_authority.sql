...
-- Establishes configuration, deterministic scope constraints, immutable
-- application history, and caller-bound Admin/Owner read authority.
...
create table public.promotions (
  ...
  constraint promotions_discount_shape check ((discount_type='fixed' and fixed_amount_sen is not null and percent_basis_points is null) or (discount_type='percent' and percent_basis_points is not null and fixed_amount_sen is null)),
...
-- Add-ons are catalogue_items(kind='addon'); FK is deliberately the same table.
create table public.promotion_addons (
  promotion_id uuid not null references public.promotions(id) on delete cascade,
  addon_item_id uuid not null references public.catalogue_items(id) on delete cascade,
...
create index promotion_addons_item_id_idx on public.promotion_addons(addon_item_id);
...
alter table public.promotion_branches enable row level security; alter table public.promotion_branches force row level security;
alter table public.promotion_items enable row level security; alter table public.promotion_items force row level security;
alter table public.promotion_variants enable row level security; alter table public.promotion_variants force row level security;
alter table public.promotion_addons enable row level security; alter table public.promotion_addons force row level security;
alter table public.promotion_order_applications enable row level security; alter table public.promotion_order_applications force row level security;
revoke all on table public.promotions,public.promotion_branches,public.promotion_items,public.promotion_variants,public.promotion_addons,public.promotion_order_applications from public,anon,authenticated;
...
as $$ begin
  ...
  if not (select private.is_admin_or_owner()) then raise exception 'Admin or Owner role required' using errcode='42501',detail='PROMOTION_ADMIN_REQUIRED'; end if;
end; $$;
...
as $$ declare v_result jsonb; begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'id',p.id,'code',p.code,'name',p.name,'description',p.description,'discountType',p.discount_type,
    'fixedAmountSen',p.fixed_amount_sen,'percentBasisPoints',p.percent_basis_points,'minimumSubtotalSen',p.minimum_subtotal_sen,
    'maximumDiscountSen',p.maximum_discount_sen,'startsAt',p.starts_at,'endsAt',p.ends_at,'priority',p.priority,
    'stackingMode',p.stacking_mode,'allowWithVoucher',p.allow_with_voucher,'requiresMember',p.requires_member,
    'globalUsageLimit',p.global_usage_limit,'perMemberUsageLimit',p.per_member_usage_limit,'isActive',p.is_active,
    ...
    'updatedAt',p.updated_at) order by p.priority,p.code),'[]'::jsonb) into v_result from public.promotions p;
  return v_result;
end; $$;
