...
-- is missing/degraded or the customer profile is disabled. No synthetic state
-- survives.
...
-- Disabled customer profile: deletion remains an account/privacy operation,
-- not an active-feature privilege. Trusted role still must be customer.
insert into auth.users (id, email, raw_user_meta_data, created_at, updated_at)
values (
  '52000000-0000-0000-0000-000000000003',
  'privacy-disabled@example.test',
  '{}'::jsonb,
  now(),
  now()
);
...
select set_config(
  'request.jwt.claims',
  jsonb_build_object(
    'sub', '52000000-0000-0000-0000-000000000003'::uuid,
    'role', 'authenticated'
  )::text,
  true
);
...
begin
  if exists (
    ...
    raise exception 'disabled customer profile survived whole-account deletion';
  end if;
  ...
  if exists (
    ...
    raise exception 'disabled customer Auth identity survived whole-account deletion';
  end if;
end;
$$;
